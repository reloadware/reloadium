...
import logging
import re
from reloadium.vendored.watchdog.utils.patterns import match_any_paths
...
class FileSystemEvent:
    ...
    def __repr__(self):
        return (
            "<%(class_name)s: event_type=%(event_type)s, " "src_path=%(src_path)r, " "is_directory=%(is_directory)s>"
        ) % (
            dict(
...
class FileSystemMovedEvent(FileSystemEvent):
    ...
    def __repr__(self):
        return (
            "<%(class_name)s: src_path=%(src_path)r, " "dest_path=%(dest_path)r, " "is_directory=%(is_directory)s>"
        ) % (
            dict(
                class_name=self.__class__.__name__,
                src_path=self.src_path,
